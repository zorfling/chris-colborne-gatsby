...
Wait, what, how? This little snippet breaks your brain. Brackets and colons and equals, oh my! What was this developer thinking? Were they just being too clever, or what?! 

Although it certainly is bracket soup, there **is** method to the madness. 
...
function doRender (
  ...
  // destructure the second parameter
  {
    ...
    // extract `store` to a variable, and if it's unset, 
    // default to the result of `configureStore()`
    store = configureStore({

      // configureStore takes as argument an object with key `reducer` (which has the value of the variable `reducer`)
      reducer,

      // and a key `middleware` with the value returned from `getMiddleware()`
      middleware: getMiddleware(),  
    })
