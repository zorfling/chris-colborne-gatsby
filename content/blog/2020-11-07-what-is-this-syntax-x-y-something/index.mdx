...
Next level is destructuring defaults. Up until now, we've been assuming the data is there. But what happens if that key _might_ be there, _might_ not? That's where defaults come into play.
...
const {
  address: { city }
} = myObject; // bah bow - cannot read property 'city' of undefined

// so let's fix that with a default
const { address: { city } = {} } = myObject; // hooray, now we're looking for city on an empty default object, which won't fail
```
