...
const reducer = (state, action) => { // highlighted
  const { status, history } = state; // highlighted
  const { type, newStatus } = action; // highlighted

  switch (type) { // highlighted
    case CHANGE_STATE: // highlighted
      let newHistory = [...history, status]; // highlighted
      if (newHistory.length > 10) { // highlighted
        newHistory.shift(); // highlighted
      } // highlighted

      return { status: newStatus, history: newHistory }; // highlighted
    case CLEAR_HISTORY: // highlighted
      return { status, history: [] }; // highlighted
    default: // highlighted
      break; // highlighted
  } // highlighted
}; // highlighted
...
export const StatusHistory = () => {
  const [state, dispatch] = useReducer(reducer, { // highlighted
    status: 'stopped', // highlighted
    history: [] // highlighted
  }); // highlighted
  const start = () => dispatch({ type: CHANGE_STATE, newStatus: 'started' }); // highlighted
  const finish = () => dispatch({ type: CHANGE_STATE, newStatus: 'finished' }); // highlighted
  const clear = () => dispatch({ type: CLEAR_HISTORY }); // highlighted
