...
It's not all bad news - it will correctly catch it if the type changes completely, for instance from a \`string\` to a \`number\`, however I'd argue that that makes it even more insidious. 

So anyway, long story short, you should avoid using the method syntax when typing your functions in TypeScript. Use the function syntax (or the call signature if you need the additional properties) and your code will be much type-safer for it!